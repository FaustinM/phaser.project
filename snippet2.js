...
//Pour qu'un élément soit fixé par la caméra, notamment le score
this.monObjet.fixedToCamera = true;
...
this.monObjetMusical = game.add.audio("surnom");
this.monObjetMusical.play();
this.monObjetMusical.stop();
...
if (this.cursors.up.isDown && this.joueur.body.touching.down) // Permet de sauter avec une velocité de -350 seulement si le joueur touche le sol
  {
      this.joueur.body.velocity.y = -nombre ou variable;
  }

this.monObjet.kill(); // Permet de tuer une variable, un objet
game.state.start('main');//Permet de redémarrer le niveau

//DES EXEMPLES DE FONCTIONS -
// Pour tuer le joueur
function joueurKill(joueur,truc qui tue){
  joueur.kill();
